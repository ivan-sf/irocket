...
class Employees
{
	...
	public function create()
	{
		...
		for ($i=0; $i < $count; $i++) { 
			...
			$documentUser1 = mysqli_real_escape_string($connect,$documentUser[$i]);
			...
			$salario = strtolower(mysqli_real_escape_string($connect,$salario[$i]));
			$fechadepago = mysqli_real_escape_string($connect,$fechadepago[$i]);
			$strlen = strlen($nameUser1) - 2;
			$userNameStrlen = substr("$nameUser1", 0, -$strlen);
			$userName = $userNameStrlen . $lastnameUser1 . date('s');
			$pass = base64_encode($documentUser1);

			$sql = "SELECT * FROM userdetails WHERE documentUser='$documentUser1' AND userdetails.range=1";
			$query = $this->con->returnConsulta($sql);
			...
			if ($query) {
				$sql = "INSERT INTO `users` (`company_idcompany`, `userName`, `password`, `stateBD`) VALUES ('$idComp', '$userName', '$pass', '1')";
						...
						if ($query) {
							...
							$query = $this->con->consulta($sql);
								if ($query) {
									...
									$mensaje = "Felicitaciones ! has registrado el empleado " . $nameUser1 . " " . $lastnameUser1 . " exitosamente.";
									$sql = "INSERT INTO `notifications` (`idnotifications`, `typeNotification`, `message`, `date_register`, `users_idusers`) VALUES ('', '18', '$mensaje', '$datetimeNot', '$idus')";
									$query = $this->con->consulta($sql);
									if ($query) {
										header("location:" . URL . "empleados/crear?success");
									}else{
							...
							$ruta = 'views/assets/images/users/default_employee.png';
							$sql = "SELECT * FROM users ORDER BY idusers DESC";
							$query = $this->con->returnConsulta($sql);
							$array = mysqli_fetch_array($query);
							$idus = $array['idusers'];
	...
	public function delete()
	{
		...
		if ($query) {
			$datetimeNot = 	date("Y-m-d G:i:s A");
			$mensaje = "Se ha eliminado un empleado ya no tendras acceso a el.";
			$sql = "INSERT INTO `notifications` (`idnotifications`, `typeNotification`, `message`, `date_register`, `users_idusers`) VALUES ('', '20', '$mensaje', '$datetimeNot', '$idUser')";
			$query = $this->con->consulta($sql);
			if ($query) {
				header("location:" . URL . "empleados?success_update");
			}else{
				echo "Error en la notificacion";
			}
		}
	...
	public function update()
	{
		...
		$pass = base64_encode(mysqli_real_escape_string($connect,$this->pass));
