...
<?php
    ...
    $sql = $con->query("SELECT * FROM `bills` WHERE stateBill=2 AND users_idusers=$iduser AND typeBill=1");
    $array = mysqli_fetch_array($sql);
    $row = mysqli_num_rows($sql);
    $idFactura = $array['idbills'];
    
    $sql = $con->query("UPDATE `bills` SET `idCliente` = '', `cliente` = 'cuantias menores', `documentUser` = '222222222' WHERE `bills`.`idbills` = $idFactura;");
